...
it('should return responseCode 200 and a specific object with a non-empty id and name', async () => {
  const response: AxiosResponse<ListOfAllObjects> = await axios.get(TestDataConfig.api.ListOfSpecificObject);
  expect(response.status).toBe(200);
  expect(Array.isArray(response.data)).toBe(true);
  expect(response.data.length).toBeGreaterThan(0);
